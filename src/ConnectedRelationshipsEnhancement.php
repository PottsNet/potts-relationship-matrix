...
final class PottsRelationshipMatrixConnectedEnhancement
{
     ...
     * to every other selected person. Blood routes are preferred. Where no blood
     * route exists, the wider visible family graph is used as a fallback.
     *
    ...
    public function calculate(array $selected, Tree $tree, array $matrix_data): array
    {
        ...
        foreach ($selected as $target_index => $target) {
            ...
            if (is_array($pair) && !empty($pair['paths'])) {
                // Include every raw path that belongs to the closest grouped
                // relationship route. This keeps both members of an ancestral
                // couple available to the merged network where appropriate.
        ...
        // Ensure every selected endpoint remains present even if one path was
        // deduplicated against a previously processed target.
        foreach ($selected as $index => $individual) {
            $relationship = $index === 0
                ? I18N::translate('Reference person')
                : (string) ($matrix_data['cells'][0][$index]['name'] ?? ($people[$individual->xref()]['relationship'] ?? I18N::translate('Related')));
            $this->addPerson($people, $individual, $index, isset($people[$individual->xref()]) && ($people[$individual->xref()]['connected'] ?? false), $relationship);
        }
        ...
        return [
            ...
            'reference' => [
                'xref' => $reference->xref(),
                'name' => strip_tags($reference->fullName()),
            ],
            'selected_count' => count($selected),
    ...
    /**
     * Add the button on every matrix page and, when requested, render a separate
     * top-down connected-family network. This view intentionally permits spouse
     * links; it is not labelled as shared ancestry.
     */
...
.potts-rm-connected-tag.relationship{border-color:var(--bs-primary,#0d6efd);font-weight:700}.potts-rm-connected-tag.bridge{border-color:var(--bs-warning,#ffc107)}
...
function build(){
    displayOptions();
    const nodeById=new Map(connectedGraph.nodes.map(node=>[node.id,node]));
    const levels=new Map(connectedGraph.nodes.map(node=>[node.id,0]));

    // Parent/child roles establish vertical generations. Partner roles keep both
    // spouses on the same row. Iterate a few times so the constraints propagate
    // through merged branches.
    const limit=Math.max(8,connectedGraph.nodes.length*3);
    for(let iteration=0;iteration<limit;iteration++){
        let changed=false;
        connectedGraph.families.forEach(family=>{
            const spouses=(family.spouses||[]).filter(id=>levels.has(id));
            const children=(family.children||[]).filter(id=>levels.has(id));
            let spouseLevel=0;
            spouses.forEach(id=>{spouseLevel=Math.max(spouseLevel,levels.get(id)||0)});
            spouses.forEach(id=>{if((levels.get(id)||0)<spouseLevel){levels.set(id,spouseLevel);changed=true}});
            const childLevel=spouses.length?spouseLevel+1:0;
            if(spouses.length){children.forEach(id=>{if((levels.get(id)||0)<childLevel){levels.set(id,childLevel);changed=true}})}
        });
        if(!changed)break;
    }

    let maxLevel=Math.max(0,...Array.from(levels.values()));
    connectedGraph.nodes.forEach(node=>{if(node.connected===false){maxLevel+=1;levels.set(node.id,maxLevel)}});

    const rows=new Map();
    connectedGraph.nodes.forEach(node=>{
        if(!cards.has(node.id))return;
        const level=levels.get(node.id)||0;if(!rows.has(level))rows.set(level,[]);rows.get(level).push(node);
    });
    ...
    let maxRowWidth=0;
    rows.forEach(nodes=>{maxRowWidth=Math.max(maxRowWidth,nodes.length*cardWidth+Math.max(0,nodes.length-1)*horizontalGap)});
    ...
    connectedGraph.families.forEach(family=>{
        ...
        if(spouses.length>=2){
            ...
            junctionX=spouses[0].cx;junctionY=spouses[0].bottom;
        }else{
            const childTop=Math.min(...children.map(item=>item.top));junctionX=children.reduce((sum,item)=>sum+item.cx,0)/children.length;junctionY=childTop-48;
        }

        if(children.length){
            const minChildTop=Math.min(...children.map(item=>item.top));
            const busY=junctionY+Math.max(28,(minChildTop-junctionY)/2);
            path('M '+junctionX+' '+junctionY+' V '+busY,'potts-rm-connected-descent');
            const left=Math.min(junctionX,...children.map(item=>item.cx)),right=Math.max(junctionX,...children.map(item=>item.cx));
            if(children.length>1)path('M '+left+' '+busY+' H '+right,'potts-rm-connected-descent');
            children.forEach(child=>path('M '+child.cx+' '+busY+' V '+child.top,'potts-rm-connected-descent'));
        }
    });
...
function applyScale(){
    let scale=1;if(fit&&fit.checked&&naturalWidth>0)scale=Math.min(1,Math.max(.42,(viewport.clientWidth-22)/naturalWidth));canvas.style.transform='scale('+scale+')';sizer.style.width=Math.max(viewport.clientWidth,naturalWidth*scale)+'px';const scaledHeight=Math.ceil(naturalHeight*scale+20);const visibleHeight=Math.max(320,Math.min(920,scaledHeight));sizer.style.height=scaledHeight+'px';viewport.style.height=visibleHeight+'px';
}
            ...
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                <div>
                    ...
                    <div class="potts-rm-note">
                        <?= I18N::translate('Connects the selected people through visible parent-child and spouse/partner links. Unlike the shared ancestry graph, this view does not require one ancestor to be shared by everyone.') ?>
                    </div>
                </div>
            </div>
            ...
            <?php else : ?>
                ...
                <div class="potts-rm-connected-summary">
                    ...
                    <?php foreach (($graph['connections'] ?? []) as $connection) : ?>
                        <span class="potts-rm-connected-chip">
                            <strong><?= htmlspecialchars((string) $connection['name'], ENT_QUOTES) ?>:</strong>
                            <?= htmlspecialchars((string) $connection['relationship'], ENT_QUOTES) ?>
                            <?php if (!empty($connection['via_name'])) : ?>
                                · <?= I18N::translate('via %s', htmlspecialchars((string) $connection['via_name'], ENT_QUOTES)) ?>
                            <?php endif ?>
                        </span>
                    <?php endforeach ?>
                </div>
                            ...
                            <div class="potts-rm-connected-cards" id="potts-rm-connected-cards">
                                <?php foreach (($graph['nodes'] ?? []) as $node) : ?>
                                    ...
                                    <div class="potts-rm-connected-card<?= !empty($node['selected']) ? ' is-selected' : '' ?><?= !empty($node['bridge']) ? ' is-bridge' : '' ?>" data-connected-node="<?= htmlspecialchars((string) $node['id'], ENT_QUOTES) ?>">
                                        <?php if (!empty($node['selected'])) : ?>
                                            <span class="potts-rm-connected-tag"><?= I18N::translate('Person %s', I18N::number((int) $node['person_number'])) ?></span>
                                            <span class="potts-rm-connected-tag relationship"><?= htmlspecialchars((string) $node['relationship'], ENT_QUOTES) ?></span>
                                        <?php elseif (!empty($node['bridge'])) : ?>
                                            <span class="potts-rm-connected-tag bridge"><?= I18N::translate('Connection branch') ?></span>
                                        <?php endif ?>
                ...
                <div class="potts-rm-connected-legend">
                    <span><span class="potts-rm-connected-legend-line"></span><?= I18N::translate('Parent-child descent') ?></span>
                    <span><span class="potts-rm-connected-legend-line partner"></span><?= I18N::translate('Spouse/partner link') ?></span>
                </div>
            <?php endif ?>
        </section>
        <?php
